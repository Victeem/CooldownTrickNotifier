...
public class CtnConfigScreen extends AbstractConfigScreen<CtnConfig> {
    ...
    protected WidgetCreator[] getWidgets(CtnConfig config) {
        return new WidgetCreator[]{
                ...
                new SliderOption("ctn.config.pitch", config.volume, d -> config.volume = (float)d, SliderOption.PERCENT_VALUE_TO_TEXT, 0, 2, 0.05),
                new InputOption("ctn.config.sound", config.sound, s -> config.sound = s, s -> Registries.SOUND_EVENT.containsId(Identifier.tryParse(s))),
